...
## 2026-09-23 任务 6：统一终端正式版本与网页验收

任务 1–5 的 PR #73、#71、#72、#74、#75 和任务 6 的 #76–#80、#85–#87 均经 required checks 正常合并。`niceweather.trade/` 当前运行精确 SHA `0c3ac70a517acb66a24f37274b64ce38663c8da3`；`/terminal` 保留选择参数跳转。发布包 705,892 字节、SHA-256 `4b1d9c36615ab2c86b108345c2466815a5b37f65985065598bcf4bdae702e0f0`。VM manifest 86 个文件哈希全部匹配，终端、KNYC 采集/HRRR/回测及双平台 Paper/Live 实例均 active；本次只重启终端。磁盘约 19 GiB 可用，既有旧静态资源列待审核清单，未删除。VM 清理事实见下节。

正式网页验证了双平台历史、当天、已挂牌未来日与档位选择，缺价格历史不回退；天气来源、分钟修订图与缺口展示；停止策略后信号持续；双平台 Live 账户仅只读认证和对账。Poly US 2026-09-22 `lt63f` 人工 Paper 以 Ask 0.01 买入一份、以最近有效市场报价 0.01 卖出一份，两笔 Nautilus 订单均 FILLED，现金 100→99.99→100、持仓 0、权益 100、费用 0。重复提交未发生，刷新后两笔订单及余额恢复。#87 修复报价更新导致卖出预检反复中断；预检在正式网页通过。提交回执和账户投影之间有短暂延迟，最终以请求 journal 和结果库核对。

最终验收记录见 PR #88。该 PR 初次运行 GitHub Actions 时因账户付款或 spending limit 提示未启动；仓库转为 public 后，精简 README 的新提交已正常触发七项 required checks，仍按保护规则合并。公开 README 只保留通用数据处理与可视化描述；代码、其他文档及 Git 历史仍随公开仓库可见。生产代码 #87 已在此前完成检查、合并和部署。

正式网页完成 Poly US 2026-09-21 单日 S1/S2/S3 与 2026-09-20–22 多日 S2 真实回测，历史选择器重载及按日/bin 切换正确。两次均零成交、权益 100，界面列出实际预测/盘口接收覆盖及陈旧天气、未核实规则等未触发原因；不能以零成交结果验证交易标记定位或盈利曲线。完整纽约市场日、最终结算、用户自行实盘交易仍待发生。各项浏览器和部署证据、当前限制见 `acceptance/knyc-task6-2026-09-22.md`。本节为最新状态，下方 2026-09-22 记录保留当时进度，不代表当前版本。

## 2026-09-22 VM 数据清理（已部署并核验）

用户批准清理 VM 市场历史、旧账本和模拟数据，天气原文须 R2 回读核验后清理，并持续自动执行。PR #81/#82/#83 均通过七项 CI 并合并，维护代码 2c0f162 已部署；KNYC 终端业务代码沿用 18cfdfc。

三份旧库天气表经 R2 逐对象回读归档后删除原库，合计 2,435,452,928 字节。旧模拟结果库从 103,559,168 缩为 69,632 字节；KNYC 清除 15 个模拟/回测运行，实盘相关记录哈希不变。KNYC feed 从 1,516,687,360 缩为 527,679,488 字节，保留 20,366 条天气事件、全部接收/修订元数据及最大序号 1,154,468，完整性检查通过。第一轮 KNYC 归档核验后清除了 1,454 个原始响应体。

旧主库从 15,555,043,328 缩为 377,626,624 字节；2026-09-23 00:57:29 UTC 核验所有保留表行数一致，原文/截图剩余均为零，quick_check=ok、空闲页为零。VM 可用空间 19,674,050,560 字节（约 18.32 GiB）。采集恢复后新原文会短暂积累，下一轮验证归档后自动清空。

KNYC 各服务及 KLGA 采集、仪表盘、终端均已恢复，两个 R2 timer 已启用。恢复后 KLGA 自动任务于 00:57:42 UTC 成功，KNYC 自动任务于 00:52:38 UTC 成功；均按约 15 分钟周期验证后清理。旧 Poly Intl 市场/runner/sandbox/backtest 服务已禁用。本轮没有真实资金操作。核验摘要见 [VM 清理记录](acceptance/vm-retention-20260922.json)，VM 原始审计记录保留于 /var/lib/nice-weather/retention-20260922。
