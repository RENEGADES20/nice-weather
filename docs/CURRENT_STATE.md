...
最后更新：2026-09-07（已结算研究数据截至纽约 09-05；09-06 已补取，抓取时未结算）

## 当前阶段

项目处于“KLGA 统一天气存储与阶段 A Shadow Runner 已部署，Tmax 重定价研究采集进入 schema v7，Repricing 数据与交互补修已部署、生产性能指标仍未达标”的阶段。

## 2026-09-07 NOAA 跨档、动态结束概率与组合配置研究

- 图文报告：`research_runs/2026-09-07-klga-conditional-alpha/REPORT.md`；复现说明与 `verify.py` 在同目录。仅新增研究文件及环境依赖，未改生产接口、采集、策略或交易权限，也未新写 R2。
- 直接回读 NOAA Hourly 页面后，13 个已结算日中 08-29、09-05 两天在 16–22 点首次新高跨档；通用 METAR 重建仅 1 天，R2 接收重建为 3 天。09-03 观测 15:51、接收 16:05；09-05 当前页面缺少早前两条普通小时记录。来源差异与接收时间差分别列示，不能将三层视为同一事件集。
- 572 天合格天气、418 天月度滚动预测；月份/时刻加天气与 HRRR 的旧 WU Brier 为 0.0732，日历基线 0.1181。旧 WU 首次结束概率超过 90% 后实际结束 91.1%，6 月仅 75%；需要校准，不能输出固定月度入场时刻。动态单档 377 个旧 WU 日平均每份毛收益 +0.0213，95% 区间跨零；当前 NOAA 13 日为 +0.1409，仍属小样本。
- 同预算 374 个旧 WU 日，等份组合最大理论复投回撤 24.9%，四分之一联合凯莉 77.4%；凯莉、概率加权相对等份的配对增益区间均跨零。用户说明凯莉仅为例子后，增加正期望等份区间组合：旧 WU 186 日、每组 +0.0353，95% 区间 [+0.0192,+0.0485]；当前 NOAA 8 日、+0.0610。保留两次区间失守及异常排除结果，补充策略尚待未来独立样本验证。
- 历史层使用名义天气时刻/HRRR 起报后一小时；机器学习尚未完成实际接收时间的 R2 全量重放。当前页面为事后版本，R2 小时子集为重建。07-11 雨后再升温为主要模型失败例；08-27 为当前规则成功例。校验和五张 PNG 已完成，报告明确价格口径与协议增补。

## 2026-09-06 KLGA 全历史 Alpha 探索完成

- 研究报告：`research_runs/2026-09-06-klga-alpha/REPORT.md`；复现入口同目录 `README.md`，包含原始响应、逐日明细、规则审计及六张 PNG。研究目录继续不进入公开仓库，生产接口、数据库与采集服务未修改。
- 发现 2025-01-22 至 2026-09-05 的 588 个历史事件、587 个市场日、4,871 个 bins；取得 9,742 个 YES/NO token 的 38,301,827 个生命周期价格点与 1,567,883 条公开 taker 成交。586 个合格去重市场日中，584 天可联合分析；573 天 WU、13 天 NOAA Hourly，不能将旧规则结果直接外推到当前规则。
- 历史观测采用 ASOS/NCEI 镜像、METAR/SPECI 与 NOAA ISD；6,457 个 HRRR 起报版本提取站点最近格点，其中 86 个 Zarr 缺失已用 NOAA GRIB2 forecast 补齐。历史层以名义观测时间、HRRR 起报后 1 小时作为可用性假设；实际接收层只使用已收到版本。
- 固定规则的时间顺序验证：升温结束单档 165 天、平均毛收益 +0.0162，95% 日期 bootstrap 区间 [-0.0168,+0.0508]；相邻组合 162 天、+0.0067，区间 [-0.0283,+0.0436]。均为证据不足，简单 HRRR 过滤器未优于共同日期的仅观测对照。
- 补充发现候选：16 点后首个小时新高跨档，在时间顺序验证段 24 天命中 22 天；两分钟内首笔公开 YES BUY 全部匹配，平均毛收益 +0.3095，区间 [+0.1769,+0.4242]。全部 24 天属于旧 WU 规则，时段筛选为补充探索；当前 R2 实际接收层仅 3 个晚间事件，尚未确认当前规则 alpha，不构成策略晋升。
- R2 235 份官方页面中 189 份含高频行，研究按标准 METAR/SPECI 时间重建小时子集，不能证明原页面选择 Hourly 模式。87 条 METAR 时间仅在研究副本按原文修正；原始记录保留。九天“次日行首次接收”确认代理的价格均为 0.9995，不能代表官方最终数据首次发布事件。
- 读取 63 个 Parquet、9 个 manifest。另在私有 R2 写入一个研究传输包 `research/klga-alpha/2026-09-06/weather-r2-8809a9ea.tar.gz`（7,006,537 字节；SHA256 见研究 README），未改变生产前缀或权限。原天气 HTTP 批次未全部下载，本轮保留了页面原文与结构化版本证据。新实时市场采集仅给出最小建议，尚未建设。
- `analysis.py --check` 与 `verify_results.py` 通过夏令时、边界、未来数据/修订、缺价、组合收益、逐日唯一性、HRRR 覆盖及 24 个成交匹配校验。保留异常排除前后结果、失败案例和初始协议中未执行的额外敏感性项目说明。

## 2026-09-06 Alpha 数据可行性与历史探索

- 本地研究报告：`research_runs/2026-09-06-alpha-feasibility/REPORT.md`；补采与 R2 归档任务 prompt：同目录 `COLLECTION_PROMPT.md`。研究区继续不进入公开仓库，运行代码、数据库 schema、R2 allowlist 和 SHADOW 均未修改。
- 已实测 08-27 至 09-04 九个完整 NYC 市场日、99 个 YES token 的历史价格，共 141946 点及目标日 7270 条公开 YES 成交记录；07-01 的 11 档也能返回历史。必须指定绝对时间窗口；`interval=max` 即使 fidelity=1 仍可能降到小时精度。09-01 约 53 分钟缺口在缩小窗口后仍存在。
- 从线上统一库只读导出天气版本，并直接 GET 校验 R2 09-04 的官方逐行 Parquet 与 manifest。四天已冻结官方标签与 Gamma 结果一致。研究使用 as-of 天气与事后评价标签分离的口径，分钟价格不视为可执行 Ask。
- 九天 16:00 当时主档均最终胜出、平均历史价格 0.8267；其中 08-31 全档价格和 1.988 为未解释异常。剔除该检查点后为 8/8、均价 0.8319。四天最终标签接收时价格均为 0.9985–0.9995，未取得接收后的 YES BUY。全部结果为小样本探索，不构成 Alpha 或可成交利润验证。
- 白天官方页面每小时采集、历史采样价格无 Ask/量、旧 tick 来源与成交字段缺失，使秒级抢单及组合容量仍不可验证。R2 市场归档仅形成补充方案，尚未部署；原数据不删除或改写。
