...
VM 已安装独立 Python 3.12 / Nautilus 1.231.0 环境，原生 38 项测试通过；Trading、Backtest 页签及两个服务已安装，LIVE 关闭。生产约 246 万条行情暴露出启动历史扫描问题，因此首次上线的模拟 worker 暂停，正在验收增量修复：最新合约元数据初始化、查询阶段排除陈旧行情、磁盘一致性回测副本。暂停期间保留原会话，不重置账户。新增积压回归后的 39 项原生测试已在 WSL 通过；VM 修复后验证及完整 KLGA 市场日观察仍未完成，PR #36 保持 draft。
