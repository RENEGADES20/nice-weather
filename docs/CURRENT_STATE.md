...
- 真实库进一步复现缩放导致范围相互回写、浏览器停止响应。范围同步改为由当前操作的图驱动另一张图，pointer/wheel/touch 捕获阶段先确定驱动图；Follow/Reset/完整窗口切换由主图驱动。两图仍按真实时间范围和共同时间基底对齐。70000 原始价格点、约 11425 个共同时间点的桌面/移动双向缩放回归通过，生产隔离复验进行中。

- PR #30、#31、#32 均经五项 CI 通过后合并。当前生产业务代码为 `90712196f1492cf63788fc7b56cdfbed75440d02`，部署时间 23:37:34 UTC；四个主服务及原 R2 timer 已恢复，519080 条旧 tick 保留。
...
- 最近完整性能验收（`62a621b`，相同前端）使用约 7.5 万价格点：冷首屏 47.083 秒，已加载 bin 切换 p95 8.252 秒，后台查询与绘图合计 p95 1.468 秒，传输 p95 2.474 秒。VM 同时运行 headless 浏览器；这些结果超过 1 秒/500ms 目标，接收到可见更新的 3 秒目标也未在此轮生产样本中独立证明。缓存、压缩、精简等值阶梯顶点及实例复用已落地，性能验收不能标为通过。
