...
## 2026-09-22 VM 数据清理授权与实现中

用户批准清理 VM 市场数据、旧账本和模拟数据；天气原始数据须核实 R2 完整归档后删除，后续同样自动清理。该授权替代此前对应数据的保留限制，实盘记录、天气解析结果、接收时间和版本证据保留。当前只完成只读盘点与本地实现，尚未清理或启用自动任务。

VM 只读盘点：根盘约 323 MiB 可用；旧统一库 15,546,994,688 字节，三份旧天气/混合库约 2.44 GB，KNYC feed 1,347,887,104 字节。KNYC feed/HRRR 因容量保护反复重启。新增回读校验后清理原文的实现及旧库天气专用导出脚本，6 项定向检查通过；待 PR、部署与真实 R2 核验。不能把 uploaded 台账或本地测试当作已完成远端核验。

## 2026-09-22 任务 6：统一入口集成（发布验收进行中）

PR #73、#71、#72、#74、#75 已按依赖顺序通过检查并正常合并，远端 main 为 307a29d。独立工作树 codex/knyc-terminal-integration 接入市场/天气、持续信号、Paper、Live 和回测组件；其他工作树与任务 5 未提交 depth.py 保留。

统一选择保存到 URL，/terminal 保留参数跳转 /。信号展示复用 inputs 与原生成交；Paper 沿用 journal 和原请求 ID；Live 账户按平台隔离。真实验收发现账户心跳持续使 Paper 预览失效，已改用账户业务字段作为条件版本。Live 注册补读 market_directory，未来市场的规则歧义仍保留。

10 项浏览器组件/入口测试及新增心跳/重连回归通过；跨任务定向测试 33 项、最新 Live/事件/发布回归 15 项通过。公开真实 Poly US 2026-09-22 市场完成一份模拟买入和卖出，资金、持仓和订单更新正确。生产发布尚未完成，后续结果见 TASK6_INTEGRATION.md。

VM 部署前 manifest 为 348123a；约 1 GB 可用，feed/HRRR 已因旧版 1 GiB 余量检查退出。用户批准仅删除一个指定 Snap 缓存文件（361205760 字节）；采集、备份和账目保留。用户已批准现有双平台凭据传输至同一 VM 的 /etc/nice-weather/credentials，初始交易关闭，真实写操作为零。
