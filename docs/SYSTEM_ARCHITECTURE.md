...
## 当前目标架构（2026-09-19，迁移中）

React/TypeScript/Vite 统一终端 → Python API/实时事件 → Kalshi、Poly US 行情适配与 Nautilus 交易引擎。天气采集 → 有版本的 as-of 输入 → KNYC 策略所需概率校准 → S1/S2/S3 共用信号函数 → 共享风险 → 模拟/实盘命令。回测复用同一信号逻辑，独立后台进程执行。

执行版 v2 在现有天气采集循环输出 prediction，不新增模型服务；本机训练的浅树 JSON 由标准库推理。Paper 记录最新天气输入、首次触发、尝试与已成交状态；盘口及风险变化可重评，重复有效输入不重复下单。回放读取固定序号上界并释放每页读事务，账户事实和回放结果仍由原 Nautilus 链路保存。

策略信号携带版本、平台、站点、合约、截止时间、概率、可执行成本和拒绝原因；订单带账户/模式/幂等请求 ID。Paper/Live 隔离，状态未知先对账。用户启用实盘，默认关闭。完成状态见 CURRENT_STATE.md。

以下为历史 KLGA 实现与设计记录；涉及 MVP 范围、候选引擎和实盘禁止的旧条款由 D-KNYC-20260919 替代。仍在运行的旧服务保持兼容，直到新终端验收完成。

## 历史记录

# 系统架构概览

Trading 统一工作台（2026-09-13）：Streamlit 读取账户投影、写入请求队列，现有 TypeScript 组件显示交易界面。公开 WebSocket → Worker 内存 L2 簿 → Nautilus L2_MBP；127.0.0.1:8766 提供展示副本，前端数据不能成为撮合依据。Live 无账户和下单权限，Paper/Backtest 独立。

实时版本 3 将原生交易/账户事实和请求回执放在同一 SQLite 事务；检查点覆盖写入，权益常规每分钟记录，成交/结算/有效性变化额外记录。不增加完整盘口或逐 tick 日志。旧原生版本先按旧规则核验重放后转换；独立旧 Paper 只读，Backtest 保持历史重放。重启取消旧挂单且不重复撮合成交。曲线/日历共用纽约日界及权益入口，缺口不插值；未存盘口的时期无法精确重建流动性。

双图时间范围同步由当前操作的图单向驱动，输入捕获阶段确定驱动图；程序化 Follow/Reset/完整窗口切换由主图驱动，避免两个范围回调持续相互触发。

十字线同步过滤布局产生的延后合成移动，保留真实输入和离开事件；minBarSpacing=0.001 允许密集日内时间点完整装入日期窗口。整日范围、双向缩放、跨图审计和鼠标离开均有浏览器回归。
...
完整快照以原始数据替换审计缓存；绘图层优先复用同起点实例，再复用同指标剩余实例，不要求不同 bin 的缺口起点相同。多余实例移除，实际空段保留；选项未变化时跳过重复布局。WeakMap 仅记录实例选项，不延长已移除实例的生命周期。

完整快照解压与 BroadcastChannel 增量共用一个 Promise 队列，应用时仍核验选择签名和基线序号。更新分段前暂时把新、旧绘图时间加入共同基底，完成后收缩为最终基底，使全局时间索引只统一重排；撤销或重写段内点使用现有实例的批量 setData，普通尾部追加仍用 update。
