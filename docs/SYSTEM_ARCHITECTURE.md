...
React/TypeScript/Vite 统一终端 → Python API/实时事件 → Kalshi、Poly US 行情适配与 Nautilus 交易引擎。天气采集 → 有版本的 as-of 输入 → KNYC 独立概率模型 → S1/S2/S3 共用信号函数 → 共享风险 → 模拟/实盘命令。回测复用同一信号逻辑，独立后台进程执行。
