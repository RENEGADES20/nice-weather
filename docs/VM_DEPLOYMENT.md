...
`scripts/update_knyc_runtime.py` 对已安装 KNYC 环境执行有限范围的终端更新，默认仅检查；只有显式 `--apply` 才写入。验证发布包、manifest、现役基线与路径，只重启终端；跨其他模块的改动拒绝应用，需单独核验影响服务。重复执行可接续相同清单的部分写入，不将旧账目恢复覆盖现役数据库。它不创建历史目录，不复制环境，不删除业务数据。
...
KNYC 当前部署：2026-09-19 16:42:11 UTC，`59e0b3e32f3cdb5ea29801ddf346a32de88481c9` 已安装至独立版本目录，六个服务运行；旧入口保留。准确路径、产物哈希、验证结果及剩余事项见 [部署记录](acceptance/knyc-vm-2026-09-19.md)。现有认证复用和公网预览仍待完成，不能将回环健康检查等同于产品验收。
